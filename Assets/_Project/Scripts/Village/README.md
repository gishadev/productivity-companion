...
```
Village/
  VillageMasterSO.cs      villager prefab, sprite pool, spacing — edit-time data
  VillageController.cs    entry point; owns the population and drives the AI
  ...
  Villagers/
    Villager.cs             one sprite; no logic
    VillagerState.cs        Idle
...
Sprite variation is a random pick from `villagerSprites` and is deliberately **not persisted**: it is
re-rolled every launch.

---

## Wandering

Two states, alternating: **Idle** for a random `idleDuration`, then **Wander** to a nearby point.
